...
SummaryRanges.prototype.addNum = function (val) {
    // let i = this.intervals.findIndex(([start, _]) => val < start);
    // let i = this.intervals.findIndex(([start, end]) => val == start);

    // if (this.intervals.findIndex(interval => interval.includes(val)) != -1) return;

    // const i = this.intervals.findIndex(([start, _]) => val < start);
    // i = i == -1 ? this.intervals.length : i;
    ...
    for (let i = 0; i < this.intervals.length; i++) {
        ...
        if (val < interval[0]) {
            ...
            if (interval.includes(val)) return;
        }
    }
    ...
    if (index == -1) {
        ...
        // if (length > 0 && val - prev[1] <= 1) {
        // if (length > 0 && val - prev[1] < 1) {
        if (prev) {
            const differenceToPrev = val - prev[1];
        ...
        const differenceToNext = next[0] - val;
        const insertBeforeNext = function () {
