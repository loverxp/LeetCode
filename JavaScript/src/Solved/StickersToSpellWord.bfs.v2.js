...
var minStickers = function (stickers, target) {
    ...
    // let states = [[makeCounter(target), stickers]];
    let states = [makeCounter(target)];
    // let states = new Set([makeCounter(target)]);
    while (states.length) {
        // console.log();
        // console.log();
        // console.log(states);
        const states2 = [];
        const keys = new Set();
        // for (const [target, stickers] of states) {
        for (const target of states) {
            for (const sticker of stickers) {
                const newTarget = new Map(target);
                // const newStickers = new Set(stickers);
                ...
                if (changed) {
                    if (newTarget.size == 0) {
                        ...
                        if (!keys.has(key)) {
                            keys.add(key);
                            // states2.push([newTarget, stickers]);
