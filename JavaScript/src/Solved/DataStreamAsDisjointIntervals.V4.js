...
SummaryRanges.prototype.addNum = function (val) {
    ...
    // while (start < end - 1) {
    // while (start < end && start != index) {
    while (low <= high) {
        // console.log({ start: low, end: high });
        // const i = Math.trunc((end - start) / 2);
        // index = Math.trunc((high - low) / 2);
        const mid = Math.trunc((high + low) / 2);
        // index = low + Math.trunc((high - low) / 2);
        const curValue = this.intervals[mid][0];
        if (val == curValue) {
            index = mid;
            break;
        }
        else if (val < curValue) {
            high = mid - 1;
        }
        else {
            low = mid + 1;
    ...
    // if (index != -1 && index < this.intervals.length && (this.intervals[index][0] == val || (index > 0 && this.intervals[index - 1][1] == val))) return;
    if (index < this.intervals.length && (this.intervals[index][0] == val || (index > 0 && this.intervals[index - 1][1] == val))) return;
    ...
    const differenceToNext = next ? next[0] - val : Number.MAX_SAFE_INTEGER;
    const differenceToPrev = prev ? val - prev[1] : Number.MAX_SAFE_INTEGER;
