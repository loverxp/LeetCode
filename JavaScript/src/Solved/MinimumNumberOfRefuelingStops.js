...
var minRefuelStops = function (target, startFuel, stations) {
    let farmosts = new Map();
    farmosts.set(-1, startFuel);
    let steps = 0;
    while (farmosts.size > 0) {
        let map = new Map();
        for (const [index, farmost] of farmosts) {
            if (farmost >= target) {
                return steps;
            }
            else {
                for (let i = index + 1; i < stations.length; i++) {
                    ...
                    if (farmost >= pos) {
                        ...
                        if (!map.has(i) || nextFarmost > map.get(i)) {
                            map.set(i, nextFarmost);
                        }
                    }
                    else {
                        break;
