...
var domino = function (m, n, broken) {
    const board = Array(m).fill(0);
    broken.forEach(([i, j]) => board[i] ^= (1 << j));
    // const dp = Array.from({ length: m }, () => Array.from({ length: n }, () => Array(Math.trunc(m * n / 2)), () => 0));
    // const dp = Array.from({ length: m }, () => Array.from({ length: n }, () => Array(1 << n), () => Array(1 << n).fill(0)));
    return dfs(0, 0, 0);
